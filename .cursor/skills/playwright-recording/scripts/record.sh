#!/usr/bin/env bash
# .cursor/skills/playwright-recording/scripts/record.sh
...
project_dir="${CURSOR_PROJECT_DIR:-${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null)}}"
if [ -z "$project_dir" ]; then
  echo >&2 "error: not inside a git repo and CURSOR_PROJECT_DIR is unset"
  exit 66
fi
cd "$project_dir"
