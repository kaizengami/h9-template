...
allowed-tools: Bash(gh issue view:*), Bash(gh issue list:*), Bash(gh search:*), Read
---
...
The skill injects this data before the model sees the prompt:

- Issue body: !`gh issue view "$ARGUMENTS" --json number,title,body,labels,state,author,createdAt,updatedAt 2>/dev/null || echo '{"error":"issue not found or gh not authenticated"}'`
- Comments (last 10): !`gh issue view "$ARGUMENTS" --json comments 2>/dev/null | jq '.comments[-10:]' || echo '[]'`
- Linked PRs: !`gh issue view "$ARGUMENTS" --json closingIssuesReferences 2>/dev/null || echo '{}'`

## Your task

Read the injected issue data above. Produce a triage doc with this
exact shape:
...
- **`gh` not authenticated.** The injected data will be empty.
