...
argument-hint: "<pattern> <replacement> [glob]"
disable-model-invocation: true
allowed-tools: Read, Glob, Grep, Edit, Write, Bash(npx:*), Bash(npx --yes ast-grep:*), Bash(npx --yes @ast-grep/cli:*), Bash(git:*), Bash(rg:*)
---
...
1. **Dry-run is mandatory.** Never run with `--update-all` on the
   first pass. Anthropic's "verify before declaring done" applies
   doubly to mass rewrites.
