...
allowed-tools: Read, Glob, Grep, Edit, Write, Bash(pnpm:*), Bash(npm:*), Bash(npx:*), Bash(node:*), Bash(git:*), Bash(gh:*), Bash(rg:*)
---
...
Subagents cannot spawn other subagents (Claude Code constraint). This
command runs in the main thread, invoking each subagent in sequence
via `@<name>` mentions. If a subagent needs to delegate further, it
must return to this command, which delegates the next step.
